...
def infer_bigpic(det_file,vis_dir, net, imglist,im_size=300, thresh=0.05,save_results=True):
    ...
    for imgpath in tqdm(imglist):
        ...
        for i in range(rows):
            for j in range(cols):
                im=big_im[i*im_size:i*im_size+im_size,j*im_size:j*im_size+im_size,:]
                img=im.copy()
